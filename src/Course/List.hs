...
filter p = foldRight f Nil
    where
        f x = (if p x then (:.) else flip const) x
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight applyOptional (Full Nil) . map (mapOptional (:.)) 
...
-- Only possible options are reverse, id, and Nil, and only reverse
