...
between o c a = o >>> a >>= (\result -> c >>> pure result)
...
hex :: Parser Char
hex = 
    replicateA 4 (satisfy isHexDigit) >>= \a ->
        case chr <$> readHex a of
            Full c -> return c
            Empty -> failed
...
-- True
eof :: Parser ()
eof = P (\s -> case s of Nil -> Result Nil (); _ -> ErrorResult (ExpectedEof s))
