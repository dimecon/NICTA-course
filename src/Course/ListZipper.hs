...
findLeft p (ListZipper ll a rl) = 
    case break p ll of
         (_, Nil)      -> IsNotZ
         (ls, r :. rs) -> IsZ (ListZipper rs r (reverse ls ++ (a :. rl)))
...
findRight p (ListZipper ll a rl) =
    case break p rl of
         (_, Nil)     -> IsNotZ
         (l, r :. rs) -> IsZ $ 
             ListZipper (reverse l ++ a :. ll) r rs
...
moveLeftLoop (ListZipper Nil a rs) =
    let (r :. rs') = reverse (a :. rs)
     in ListZipper rs' r Nil
...
moveRightLoop (ListZipper ls a Nil) =
    let (l :. ls') = reverse (a :. ls)
     in ListZipper Nil l ls'
...
moveLeftN n lza
    | n < 0     = moveRightN (negate n) lza
    | otherwise = go (n-1) (moveLeft lza)
        where
            go _ IsNotZ  = IsNotZ
            go 0 l       = l
            go x (IsZ l) = go (x-1) (moveLeft l)
...
moveRightN n lza 
    ...
    | otherwise = go (n-1) (moveRight lza)
        where
            go _ IsNotZ  = IsNotZ
            go 0 l       = l
            go x (IsZ l) = go (x-1) (moveRight l)
...
-- nth n lz = moveRightN n -<< fromList (toList lz) -- suboptimal reference
nth n _ | n < 0 = IsNotZ
nth n lz = case moveLeftN' n lz of
                Left x  -> moveRightN (n - x) lz
                Right l -> moveLeftN (length . lefts $ l) lz
...
insertPushLeft a (ListZipper l b r) = ListZipper (b :. l) a r
...
insertPushRight a (ListZipper l b r) =
    ListZipper l a (b :. r)
...
instance Apply ListZipper where
  (ListZipper fl f fr) <*> (ListZipper al a ar) =
      ListZipper (zipWith ($) fl al) (f a) (zipWith ($) fr ar)
...
instance Extend ListZipper where
  f <<= lz = ListZipper (unfoldr (g moveLeft) (moveLeft lz)) (f lz) (unfoldr (g moveRight) (moveRight lz))
      where
          g _ IsNotZ     = Empty
          g fn (IsZ lz') = Full (f lz', fn lz') 
